...
void block_erase(uint32_t addr)
{
    for(int i=0;i<16;i++)
  {
    W25QXX_Erase_Sector(addr+i*4096);
  }
}
void block_erase_hadware(uint32_t Dst_Addr)
{
    ...
    W25QXX_Wait_Busy();
    W25QXX_CS_L(); //使能器件
    W25QXX_SPI_ReadWriteByte(W25X_BlockErase);      	//发送扇区擦除指令
    W25QXX_SPI_ReadWriteByte((uint8_t)((Dst_Addr) >> 16));  	//发送24bit地址
    W25QXX_SPI_ReadWriteByte((uint8_t)((Dst_Addr) >> 8));
    W25QXX_SPI_ReadWriteByte((uint8_t)Dst_Addr);
    W25QXX_CS_H();  //取消片选
...
/*
 uint8_t ccc[20];
  W25QXX_Write_NoCheck("啊啊啊",0x030000,6);
  W25QXX_Read(ccc, 0x030000, 6);
  printf("0x%x 0x%x 0x%x 0x%x 0x%x 0x%x",ccc[0],ccc[1],ccc[2],ccc[3],ccc[4],ccc[5]);
*/

/*
汉字库相关代码请移步bsp_oled.h。
烧写汉字库相关代码请参考作者github仓库https://github.com/HAPLIFE6/oled_extern_Chinese
相关引脚略有改动：以本项目为准。
*/
